...
bool FTriangulator::Triangulating(FTriangle& triangle,std::vector<FVertex>&vBuffer, std::vector<FVertex>& points, std::unordered_map<FIndex, std::unordered_set<FIndex>>& neighborMapFrom3, std::vector<FTriangle>& triangles)
{
 ...
 //       pointsBuffer.push_back(point);
	...
	triangulator.SetEdges(points, &neighborMapFrom3);
	if (!triangulator.ReTriangulate())
		return false;

    FVec3 normal = Normal(triangle,vBuffer.data());
    std::vector<std::vector<FIndex>> trianglesIndex = triangulator.GetTriangles(); 
    int size = vBuffer.size();
    for (int i=0;i<points.size();i++) 
            vBuffer.push_back(points.at(i));
    for (auto tri : trianglesIndex) {
        tri[0] = tri[0] + size;
        tri[1] = tri[1] + size;
        tri[2] = tri[2] + size;
...
int FTriangulator::AttachPointToTriangleEdge( FVec2& point)
{
    ...
    for (int i = 0; i < 3; i++) {
        int j = (i + 1) % 3;
        FFLOAT s = (m_points[i].X * m_points[j].Y + m_points[j].X * point.Y + point.X * m_points[i].Y -
            m_points[j].X * m_points[i].Y - point.X * m_points[j].Y - m_points[i].X * point.Y);
